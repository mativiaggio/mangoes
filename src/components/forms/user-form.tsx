...
const UserForm = ({ id, type, subAccounts, userData }: Props) => {
  ...
  const form = useForm<z.infer<typeof userDataSchema>>({
    ...
    defaultValues: {
      ...
      role: userData ? userData.role : data?.user?.role || "SUBACCOUNT_GUEST",
    },
  });
  ...
  const onSubmit = async (values: z.infer<typeof userDataSchema>) => {
    ...
    if (userData || data?.user) {
      ...
      authUserData?.Agency?.SubAccount.filter((subacc) =>
        ...
        await saveActivityLogsNotification({
          agencyId: undefined,
          description: `Información actualizada de ${userData?.name}`,
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Mofica la información de tu cuenta</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-1">
                  ...
                  <FormControl>
                    <Input required placeholder="Nombre completo" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      if (
                        ...
                        setRoleState(
                          "Debe tener subcuentas para asignar acceso de subcuenta a los miembros del equipo."
                        );
                      } else {
                        setRoleState("");
                      }
            ...
            {authUserData?.role === "AGENCY_OWNER" && (
              <div>
                ...
                <FormLabel>Permisos del usuario</FormLabel>
                <FormDescription className="mb-4">
                  Puede otorgar acceso a subcuentas a los miembros del equipo
                  activando el control de acceso para cada subcuenta. Esto solo
                  es visible para los propietarios de la agencia.
                </FormDescription>
